...
<script>
    async function FillUpUpdateForm(id) {
        ...
        showLoader();
        let res = await axios.get(`/api/admin/brands/${id}`);
        res = res.data[0];
        hideLoader();
        document.getElementById('brandNameUpdate').value = res['name'];
    ...
    async function update() {
        ...
        if (brandName.length === 0) {
            ...
            showLoader();
            let res = await axios.post(`/api/admin/brands/${id}`, formData, config)
            console.log(res);
            hideLoader();
